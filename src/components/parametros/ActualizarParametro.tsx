...
import {
  ParametroSchema,
  TypeParametroSchema,
} from "@/validationSchemas/ParametroSchema"
...
const ActualizarParametro = ({
  ...
  setParametros?: Dispatch<SetStateAction<TypeParametroSchema[]>>
}) => {
  ...
  const initialValues: TypeParametroSchema =
  ...
  const onSubmit = async () => {
    ...
    async function actualizarParametro() {
      ...
      if (
        ...
        parametro.tipo_campo === "radio"
      )
        formData.append("opciones", parametro.opciones || "")
      ...
      if (setParametros) {
        setParametros((prev) =>
          prev.map((p) => (p.id === parametro.id ? parametro : p))
        )
      }
      methods &&
        methods.setValue(
          "parametros",
          methods
            .getValues("parametros")
            .map((p: TypeParametroSchema) =>
              p.id === parametro.id ? parametro : p
            )
        )
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogPortal>
          <DialogContent
            className="sm:max-w-screen-sm md:max-w-screen-md top-[50%] -bottom-[40%] overflow-y-auto px-2 py-4 md:px-20 md:py-12"
          >
            <DialogHeader>
              <DialogTitle>Actualizar parametro</DialogTitle>
              <DialogDescription>
                Los cambios afectaran a TODAS las calculadoras que usen este
                parametro.
              </DialogDescription>
            </DialogHeader>
